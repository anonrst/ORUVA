...
contract GemJoin is Auth, CircuitBreaker, ReentrancyGuard {
     ...
     */
    function join(address _user, uint _wad) external notStopped {
        if (int(_wad) <= 0) revert GemJoin_OverFlow();

        // Update user collateral in Vat
        cdpEngine.modifyCollateralBalance(collateralType, _user, int(_wad));
     ...
     */
    function exit(address _user, uint _vad) external notStopped nonReentrant {
        if (_vad >= 2 ** 255) revert GemJoin_OverFlow();

        // Update user collateral in Vat
        cdpEngine.modifyCollateralBalance(collateralType, _user, -int(_vad));
